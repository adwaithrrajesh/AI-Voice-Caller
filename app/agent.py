...
def parse_call_context(metadata: str | None) -> dict:
    ...
    return {
        "name": ctx.get("name") or settings.prospect_name,
        "phone": ctx.get("phone"),
...
class SalesAgent(Agent):
    ...
    async def tts_node(
        ...
        Validated to [1.0, 3.0] in config.py.
        """
        gain = settings.tts_gain
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    # Validate the env we actually need for an AgentSession run. This will
    # SystemExit with a clear message if SARVAM_API_KEY is missing.
    ...
    tts = sarvam.TTS(
        ...
        speaker=settings.tts_speaker,   # default 'shubh'; override via TTS_SPEAKER env var
        pace=1.0,
